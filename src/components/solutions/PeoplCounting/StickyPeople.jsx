...
export default function StickyPeople() {
  const data = [
    {
      ...
      child: "Typical Application",
      high: "Highlight",
      advan: "Advanced Feature",
      pr: "Product",
      qa: "Q&A",
    },
  ];
  ...
  return (
    <>
      ...
      <div className="flex flex-wrap justify-between items-center gap-2 w-full mx-auto lg:w-11/12">
        <div className="flex flex-wrap gap-2 w-full p-2 md:w-8/12">
          {data.map((item, index) => (
            <div
              key={index}
              className="flex gap-10 text-[#00667C] items-center flex-wrap justify-around"
            >
              <h1
                className="text-center cursor-pointer hover:text-[#7CCA9A]"
                onClick={() => handleScroll("why-people")}
              >
                {item.title}
              </h1>
              <p
                className="text-center cursor-pointer hover:text-[#7CCA9A]"
                onClick={() => handleScroll("typical-app")}
              >
                {item.child}
              </p>
              <p
                className="text-center cursor-pointer hover:text-[#7CCA9A]"
                onClick={() => handleScroll("highlight-people")}
              >
                {item.high}
              </p>
              <p
                className="text-center cursor-pointer hover:text-[#7CCA9A]"
                onClick={() => handleScroll("explore-advanceds")}
              >
                {item.advan}
              </p>
              <p
              ...
              <p
                className="text-center cursor-pointer hover:text-[#7CCA9A]"
                onClick={() => handleScroll("form-submit")}
              >
                {item.qa}
              </p>
            </div>
        ...
        <div className="flex gap-3 flex-wrap">
          <span className=" text-[#00667C] hover:text-[#7CCA9A] cursor-pointer">
            Home
          </span>
          <span className=" text-[#00667C] hover:text-[#7CCA9A] cursor-pointer">
            Solution
      ...
      {/* Sections with IDs */}
      <section id="why-people">
        <WhyPeople />
      </section>
      <section id="typical-app">
        <TypicalApp />
      </section>
      <section id="highlight-people">
        <HighlightPeople />
      </section>
      <section id="explore-advanceds">
        <ExploreAdvanceds />
      </section>
      <section id="comprehensive">
        <Comprehensive title={"Comprehensive People Counters"} />
      </section>
      <section id="success-stories">
        <SuccessStories />
      </section>
      <section id="discover-fea">
        <DiscoverFea
          title={
            "Discover the Future of Smart Sensing: Milesight People Sensing Series"
          }
          bgimage={"/PeopleCounting/child.jpg"}
          mouse={"/PeopleCounting/mouse.png"}
          btn={"Learn More"}
        />
      </section>
