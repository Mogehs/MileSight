...
export default function PairUp() {
    ...
    return (
        <div className="w-full h-fit mt-4 p-4">
            <h1 className='text-center font-semibold md:text-4xl mb-4 text-[#00667C]'>Pair up Our Products to Unlock Features</h1>
            <div className="w-full mx-auto sm:w-11/12 h-fit sm:mt-6 mt-5 grid grid-cols-1 md:grid-cols-2 gap-4">
                {data.map((item, index) => (
                    ...
                    >
                        {item.free && (
                            <img src={item.free} alt="Free" className="absolute top-0 right-0 w-12 h-12" />
                        )}
                        <img src={item.img} alt="missing" className="w-32 h-auto" />
                        <div className="mt-2 gap-2 w-full">
                            <h1 className="text-xs sm:text-xl font-semibold mt-2 text-[#00667C]">{item.title}</h1>
                            {item.title2 && <h1 className="text-lg font-semibold mt-2 text-[#00667C]">{item.title2}</h1>}
                            ...
                            <div className='flex justify-between items-center mt-4'>
                                {item.learn && (
                                    <button className='text-[#00667C] text-xs sm:text-sm font-semibold hover:underline'>
                                        {item.learn}
                                    </button>
                                )}
                                {item.btn && (
                                    <button className='text-xs lg:text-lg rounded-2xl bg-[#00667C] text-white hover:bg-[#7CCA9A] hover:text-[#00667C] px-4 py-2 mt-2 scale-[0.6] transform transition-all ease-in delay-100 hover:scale-[0.7] cursor-pointer'>
                                        {item.btn}
                                    </button>
                                )}
                            </div>
