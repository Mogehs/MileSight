...
export default function Faq() {
    ...
    return (
        <div className="w-full md:w-8/12 p-2 mx-auto mt-10">
            <h1 className=" md:text-4xl text-2xl font-semibold text-center mb-4 text-[#00667C]">FAQs on IAQ Sensors </h1>
            {data.map((item, index) => (
                <div key={index} className="mb-2 border cursor-pointer border-gray-300 rounded-md">
                    <button
                        onClick={() => toggleAccordion(index)}
                        className="w-full p-3 flex justify-between cursor-pointer items-center bg-white hover:bg-gray-300 transition"
                    >
                        <span className="text-xs md:text-lg font-medium text-[#00667C]">{item.title}</span>
