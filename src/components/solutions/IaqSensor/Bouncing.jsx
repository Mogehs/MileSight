...
export default function Bouncing() {
  ...
  return (
    <div
      className="h-full w-full bg-cover md:px-0 py-10 mt-10 px-10"
    ...
    >
      <div className="w-full sm:w-11/12 mx-auto">
        <div className="text-center text-[#7CCA9A] flex flex-col gap-3">
          ...
          <p className="text-xs">
            Educational stages. Check how we can help you to smash stale or
            polluted air adversely.
          </p>
        </div>
        <div className="mt-2 md:mt-8">
          {bounce.map((item, ind) => (
            <div
              key={ind}
              className="w-full  mx-auto md:w-[70%] flex justify-center relative"
            >
              <img
                src={item.img}
                alt=""
              ...
              <div className="md:w-[80%] lg:w-[50%] mx-6 absolute top-0 left-0">
                ...
                <button className="text-xs lg:text-lg rounded-2xl bg-[#00667C] text-white hover:bg-[#7CCA9A] hover:text-[#00667C] px-4 py-2 mt-2 scale-[0.9] transform transition-all ease-in delay-100 hover:scale-[1] cursor-pointer">
                  <a href="/resources/academy/product-training">{item.watch}</a>
                </button>
              </div>
            </div>
          ))}
