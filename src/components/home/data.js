const slides = {
    "AM Series": {
        images: [
            "/iot-sensing/pic-1.png",
            "/iot-sensing/pic-2.png",
            "/iot-sensing/pic-3.png",
        ],
        title1: ["AM102/AM102L", "AM103/AM103L", "AM307/AM308/AM319"],
        title2: ["2-in-1 IAQ Sensor", "3-in-1 IAQ Sensor", "9-in-1 IAQ Sensor"],
    },
    "People Sensing Series": {
        images: [
            "/iot-sensing/2-pic1.png",
            "/iot-sensing/2-pic2.png",
            "/iot-sensing/2-pic3.png",
        ],
        title1: ["VS121", "VS125", "VS133"],
        title2: [
            "AI Workplace Occupancy Sensor",
            "AI Stereo Vision People Counter",
            "AI ToF People Counting Sensor",
        ],
    },
    "WT Series": {
        images: [
            "/iot-sensing/3-pic1.png",
            "/iot-sensing/3-pic2.png",
            "/iot-sensing/3-pic3.png",
        ],
        title1: ["WT101", "WT201", "WT30x", "Compatibility"],
        title2: [
            "Smart Radiator Thermostat",
            "Smart Thermostat",
            "Smart Fan Coil Thermostat",
            "Checker",
        ],
    },
    "GS Series": {
        images: ["/iot-sensing/4-pic1.png", "/iot-sensing/4-pic2.png"],
        title1: ["GS101", "GS301"],
        title2: ["Gas Sensor", "Bathroom Odor Detector"],
    },
    "CoWork Series": {
        images: [
            "/iot-sensing/5-pic1.png",
            "/iot-sensing/5-pic2.png",
            "/iot-sensing/5-pic3.png",
        ],
        title1: ["WT101", "WT201", "WT30x", "Compatibility"],
        title2: [
            "Smart Radiator Thermostat",
            "Smart Thermostat",
            "Smart Fan Coil Thermostat",
            "Checker",
        ],
    },
    "WTS Series": {
        images: ["/iot-sensing/6-pic.png"],
        title1: ["WTS506"],
        title2: ["IoT Weather Station"],
    },
    "TS Series": {
        images: [
            "/iot-sensing/7-pic1.png",
            "/iot-sensing/7-pic2.png",
            "/iot-sensing/7-pic3.png",
        ],
        title1: ["WT101", "WT201", "WT30x", "Compatibility"],
        title2: [
            "Smart Radiator Thermostat",
            "Smart Thermostat",
            "Smart Fan Coil Thermostat",
            "Checker",
        ],
    },
    "EM300 Series": {
        images: [
            "/iot-sensing/8-pic1.png",
            "/iot-sensing/8-pic2.png",
            "/iot-sensing/8-pic3.png",
        ],
        title1: ["WT101", "WT201", "WT30x", "Compatibility"],
        title2: [
            "Smart Radiator Thermostat",
            "Smart Thermostat",
            "Smart Fan Coil Thermostat",
            "Checker",
        ],
    },
    "EM400 Series": {
        images: [
            "/iot-sensing/9-pic1.png",
            "/iot-sensing/9-pic2.png",
            "/iot-sensing/9-pic3.png",
        ],
        title1: ["WT101", "WT201", "WT30x", "Compatibility"],
        title2: [
            "Smart Radiator Thermostat",
            "Smart Thermostat",
            "Smart Fan Coil Thermostat",
            "Checker",
        ],
    },
    "EM500 Series": {
        images: [
            "/iot-sensing/10-pic1.png",
            "/iot-sensing/10-pic2.png",
            "/iot-sensing/10-pic3.png",
        ],
        title1: ["WT101", "WT201", "WT30x", "Compatibility"],
        title2: [
            "Smart Radiator Thermostat",
            "Smart Thermostat",
            "Smart Fan Coil Thermostat",
            "Checker",
        ],
    },
    "AT Series": {
        images: ["/iot-sensing/11-pic.png"],
        title1: ["WT101"],
        title2: ["Smart Radiator Thermostat"],
    },
    "CT Series": {
        images: ["/iot-sensing/12-pic1.png", "/iot-sensing/12-pic2.png"],
        title1: ["WT101", "WT201"],
        title2: ["Smart Radiator Thermostat", "Smart Thermostat"],
    },
};

export default slides;